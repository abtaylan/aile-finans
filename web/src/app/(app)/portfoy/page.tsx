...
import { getLatestSellingRates } from "@/lib/exchange-rates";
...
const currencyAssets = (assets ?? []).filter((a) => a.asset_type === "currency");
const currencyRates = await getLatestSellingRates(
supabase,
currencyAssets.map((a) => a.symbol)
);
const tcmbAssetIds: string[] = [];
for (const a of currencyAssets) {
if (a.symbol in currencyRates) {
latestPrices[a.id] = currencyRates[a.symbol];
tcmbAssetIds.push(a.id);
}
}
