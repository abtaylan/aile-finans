...
{ label: "Portfoy (altin/doviz/fon) piyasa degeri", value: portfolioValue },
...
<div>
<h1 className="text-xl font-semibold text-[var(--text-primary)]">Zekat</h1>
<p className="text-sm text-[var(--text-secondary)]">
Guncel varliklarina gore canli zekat hesaplamasi (altin nisabi esas alinir).
</p>
</div>
...
<div>
<p className="text-sm text-[var(--text-secondary)]">Kalan Zekat Borcu</p>
<p className="text-2xl font-semibold text-[var(--text-primary)]">
{formatCurrency(remainingZakat)}
</p>
{paymentsTotal > 0 && (
<p className="text-xs text-[var(--text-secondary)]">
Toplam {formatCurrency(zakatDue)} icinden {formatCurrency(paymentsTotal)} odendi
</p>
)}
</div>
</div>
...
Ziynet esyasi (altin) matraha dahil edilsin
</label>
</div>
</div>
...
<span className="text-[var(--text-secondary)]">Dusulebilir borc (aylik taksitler)</span>
...
{" · "}Nisab (85gr altin): <strong className="text-[var(--text-primary)]">{formatCurrency(nisabValue)}</strong>
</span>
<span
...
>
{isAboveNisab ? "Nisabin uzerinde" : "Nisabin altinda"}
</span>
</div>

<div className="mt-4 flex flex-col gap-3 rounded-lg border border-[var(--border)] p-3">
<p className="text-sm font-medium text-[var(--text-primary)]">Havl (1 Hicri Yil)</p>
...
Servetin nisaba ulastigi tarih
</label>
<Input
id="hawlStartDate"
name="hawlStartDate"
...
{hawlDateInput ? (
<p className="text-xs text-[var(--text-secondary)]">
Hicri karsiligi: <strong>{hijriDateLabel(hawlDateInput)}</strong>
{" · "}
{hawlComplete ? (
<span className="text-[var(--good)]">
Havl {formatDate(completionDate!)} tarihinde tamamlandi - zekat farz.
</span>
) : (
<span>
Havl {formatDate(completionDate!)} tarihinde tamamlanacak (yaklasik {daysLeft} gun
kaldi, Hicri: {hijriDateLabel(completionDate!)}).
</span>
)}
</p>
) : (
<p className="text-xs text-[var(--text-muted)]">
Tarih girilmezse zekat farz olma durumu MUHAFAZAKAR olarak &quot;tamamlanmadi&quot; kabul
edilir.
</p>
)}
</div>
...
Not: Altin fiyati manuel olarak Portfoy sayfasindan guncellenen son &quot;Gram Altin&quot;
fiyati esas alinir (su an {formatCurrency(goldPricePerGram)}/gr). Hicri tarih
donusumu tablo bazli bir yaklasik algoritma kullanir (~1 gun sapma olabilir);
...
Bu Yil Yapilan Zekat Odemeleri
</CardTitle>
<ZakatPaymentDialog />
</CardHeader>
...
<p className="text-xs text-[var(--text-secondary)]">
{LOAN_TYPE_LABELS[l.loan_type]} · Bitis {l.end_date}
</p>
</div>
</div>
<div className="flex items-center gap-2">
